...
# 模块层准备 TTS 临时目录（project 根下 tmp_tts）
_BACKEND_DIR = os.path.dirname(os.path.abspath(__file__))
_PROJECT_ROOT = os.path.abspath(os.path.join(_BACKEND_DIR, ".."))
TTS_DIR = os.path.join(_PROJECT_ROOT, "tmp_tts")
os.makedirs(TTS_DIR, exist_ok=True)

def _cleanup_old_tts_files(max_age_seconds=300):
    """定期清理过期的 tts 文件（避免磁盘无限增长）"""
    try:
        now = time.time()
        for p in Path(TTS_DIR).glob("tts_*.wav"):
            try:
...
@app.route("/api/ai_teacher/tts", methods=["POST"])
def api_tts():
    """
    接收 JSON: {"text": "...">
    返回: {"url": "/api/ai_teacher/tts_file/<filename>.wav"}
    使用本地 pyttsx3（SAPI5）生成 wav 文件（Windows 下可选女声）。
    """
    data = request.get_json(force=True, silent=True) or {}
    ...
    # 清理旧文件（异步/定期清理也可，这里每次请求做一次轻量清理）
    _cleanup_old_tts_files(max_age_seconds=300)

    if not TTS_AVAILABLE:
        return jsonify({"error": "pyttsx3 not available on server"}), 500

    # 生成唯一文件名
    fname = f"tts_{int(time.time())}_{uuid.uuid4().hex}.wav"
    ...
    # 返回可下载/播放的相对URL
    return jsonify({"url": f"/api/ai_teacher/tts_file/{fname}"}), 200

@app.route("/api/ai_teacher/tts_file/<path:fname>", methods=["GET"])
def api_tts_file(fname):
    """提供生成的 tts wav 文件（仅用于开发/本地）"""
    # 简单路由安全检查：只允许 tmp_tts 下的文件
    requested = os.path.normpath(os.path.join(TTS_DIR, fname))
    if not requested.startswith(os.path.normpath(TTS_DIR)):
        return jsonify({"error": "forbidden"}), 403
    if not os.path.exists(requested):
        return jsonify({"error": "file not found"}), 404
    return send_from_directory(TTS_DIR, os.path.basename(requested))
